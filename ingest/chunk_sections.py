...
# Pattern to match Section header, e.g., "27. Termination of employment by employer otherwise than by dismissal.-" or "Section 27. Retrenchment.-"
SECTION_PATTERN = re.compile(r'^\s*(?:Section\s+)?(\d{1,3}[A-Z]?)\.\s+([^.\n\-]{3,})(?:\.|\-|\.\-)?', re.IGNORECASE)

# Pattern to match sub-sections, e.g., "(1)", "(2)", "(3)"
SUBSECTION_PATTERN = re.compile(r'^\s*\((\d+)\)\s+')

# Pattern to match Schedule headers
...
def split_large_section(text: str, section_num: str, section_title: str, max_tokens: int = 800) -> list:
    """
    If a section's text exceeds max_tokens (~3200 chars), split at sub-section boundaries (1), (2).
    Prepend section title and number to preserve standalone legal context.
    """
    if len(text) < max_tokens * 4:
        return [text]
        
    logger.info(f"Section {section_num} exceeds target length ({len(text)} chars). Splitting along sub-sections...")
...
def parse_and_chunk(raw_text: str, doc_name: str, law_version_date: str) -> list:
    """
    Parses statutory text, extracts Chapters and Sections, and returns chunk dictionaries.
    """
    chunks = []
    pages = raw_text.split("--- PAGE BREAK ---")
    
    current_chapter = "General / Preliminary"
        ...
        if current_section_num is not None:
            ...
            if section_text:
                ...
                for idx, sub_txt in enumerate(sub_texts):
                    chunk_id = f"sec_{current_section_num}"
                    if len(sub_texts) > 1:
                        chunk_id += f"_part{idx+1}"
    ...
    logger.info(f"Chunking complete. Created {len(chunks)} section chunks.")
    return chunks
